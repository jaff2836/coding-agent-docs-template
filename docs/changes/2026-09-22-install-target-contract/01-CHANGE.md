...
- **Decision:** [PROJECT §8 D-010](../../00-PROJECT.md)
- **Approval:** Chae Sangwon, 2026-09-23 사용자 대화 — A안 권고 뒤 계속 진행 요청
...
- 공개된 `v2.3.1` asset은 변경하지 않고 이후 release에서만 수정합니다.

### 1.5 미해결 질문

없습니다. A안, 즉 `install`은 존재하지 않는 경로나 빈 디렉터리만 허용하고
기존 저장소에는 읽기 전용 `adopt`를 사용하는 계약을 승인했습니다.
...
|---|---|---|---|---|
| R-001 | 대상이 존재하지 않는 경로나 읽을 수 있는 빈 디렉터리인지 쓰기 전에 검사합니다. | 존재하지 않는 대상, 빈 디렉터리, 겹치지 않는 `existing.txt`가 있는 디렉터리 | 앞의 두 경우만 성공하고 비어 있지 않거나 emptiness를 확인할 수 없는 대상은 아무것도 쓰지 않습니다. | CLI·함수 회귀와 before/after tree snapshot |
...
- **A안 — 비어 있지 않은 대상을 전부 거부(선택):** root landing README의 강한
...
A안에 따라 release와 archive 검증을 마친 뒤 첫 파일을 쓰기 전에 대상 root가
...
`docs/TEMPLATE_GUIDE.md` §2입니다. 기존 저장소에는 `adopt`를 권장하는 이유와
대상 보존 경계를 유지합니다.

### 2.4 상위 설계에 미치는 영향

D-010의 A안은 D-002의 비파괴 installer와 D-006의 새 프로젝트 `install`·기존
저장소 `adopt` 역할을 명확히 하며 대체하지 않습니다.
...
| 선택이 잘못됨 | 새 프로젝트 또는 기존 저장소 흐름 불편 | D-010을 명시적으로 supersede하고 후속 release에서 계약·코드·문서를 함께 변경 |

### 2.6 완료 조건과 미해결 사항

미해결 사항은 없습니다. branch 완료는 관련 installer 회귀, 전체 unittest, root
docs, stable locale, en·ko export·artifact 검사와 `git diff --check` 통과입니다.
통합과 release 포함 여부는 별도로 기록하며 공개된 `v2.3.1`의 완료 근거를 소급
변경하지 않습니다.
...
- 2026-09-23: 사용자가 A안 권고 뒤 계속 진행을 승인해 D-010으로 확정하고
  T-016 구현을 시작했습니다.
