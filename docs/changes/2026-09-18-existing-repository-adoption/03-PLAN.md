...
- [x] **W-004 `v2.1.0` 게시와 consumer E2E**
  ...
  - 결과·근거: SPEC §6의 공개 완료와 지원 완료 조건을 모두 충족했습니다.
    ...
    - **Codex:** 처음에는 ChatGPT 계정 사용량 한도로 실행하지 못했습니다. 사용자 승인에 따라 API key 로그인으로 바꿔 Codex CLI 0.155.0 `exec -s read-only`로 다시 실행했습니다. 도구·명령 호출 0회로 probe 값, model-invocable skill `design`·`project-analysis`, 한국어를 답했습니다. `review-round`는 `allow_implicit_invocation: false`로 목록에서 빠졌습니다. 명시적 `$design` 호출에서는 skill 본문의 `docs/01-DESIGN.md §1`을 답했습니다.
