...
없습니다. candidate의 실제 draft 성공 경로는 다음 release의 draft가 존재할 때 최초 운영 검증합니다. 이 작업을 위해 임의의 draft를 만들지는 않습니다.
...
| R-004 | candidate는 기존 draft의 상태와 asset을 확인합니다. | 사람이 만든 GitHub draft | `draft=true`, `immutable=false`, 정확한 tag이며 local package 5개와 inventory·byte가 같을 때만 통과 | release shape·download·orchestration unit test, 다음 draft 운영 검증 |
...
| R-006 | published는 모든 공식 locale과 두 version selector를 공개 경로로 검증합니다. | `latest`·exact version, `en`·`ko` | `list-locales`·`install`·`export`·`adopt`와 tag source export가 같고 artifact checker가 통과하며 install 충돌·adopt 대상은 불변 | materialization unit test, `v2.1.0` 실환경 검증 |
...
- `candidate`: clean exact-source gate → 두 번 package → remote `main`·annotated tag 확인 → 기존 draft metadata·asset byte 확인
- `published`: clean exact-source package → 같은 remote ref 확인 → immutable Latest metadata·asset byte 확인 → `latest`와 exact version의 locale별 공개 `list-locales`·`install`·`export`·`adopt` 확인
...
브랜치 구현 완료는 새 unit test, 전체 unittest, root docs, stable locale, Python compile과 `git diff --check` 통과입니다. 공개 경로는 immutable `v2.1.0`의 exact checkout에서 실환경 `published` 검증으로 확인합니다. 실제 `candidate` 성공 경로는 다음 release draft에서 publish 전에 확인해야 하며, 그 실행 전에는 운영 검증 완료로 간주하지 않습니다. Origin 통합은 별도 commit·push·merge 후 확인합니다.
