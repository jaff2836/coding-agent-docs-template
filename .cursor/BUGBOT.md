...
non-blocking finding도 보고합니다. 유용한 맥락입니다. 다만 병합 게이트는 blocking finding에만 작동합니다. 작성자가 이미 이월 작업으로 추적 중인 결함은 반드시 non-blocking입니다. 아래 「확정된 설계 결정과 승인된 deferral」에 기록된 항목을 finding과 대조하세요. 지침이 여러 PR에 걸쳐 의도적으로 단계화했다고 밝힌 설계는 다시 논쟁하지 않습니다.
...
`docs/REVIEW.md` §9 Accepted Deferrals와 `docs/PLAN.md` §8 Decisions 중 리뷰 판정에 영향을 주는 항목을 여기에 복제합니다. 여기 있는 결정을 **되돌리라는** 지적은 새 정보가 아니므로 보고하지 않습니다. 결정의 **전제가 깨졌다는 증거**(문서가 약속한 것과 코드가 실제로 하는 것이 다름, 결정이 가정한 조건이 더 이상 성립하지 않음)는 새 정보이므로 보고합니다.
